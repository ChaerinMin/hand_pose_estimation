...
@PIPELINES.register_module()
class ToTensorAndHalf:
    """Transform image to Tensor and convert it to half precision.
    ...
    def __call__(self, results):
        if isinstance(results['img'], (list, tuple)):
            # Convert each image in the list or tuple to a tensor and to half precision
            results['img'] = [F.to_tensor(img).to(torch.half) for img in results['img']]
        else:
            # Convert the single image to a tensor and to half precision
            results['img'] = F.to_tensor(results['img']).to(torch.half)
